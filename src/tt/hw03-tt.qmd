...
The previous visualization was revised in order to improve readability.

:::

```{r}
p1 <- ggplot(proteins, aes(x = reorder(protein, -n), y = n, fill = protein)) + 
  geom_col() + 
  theme_classic() + 
  theme(legend.position = "none") +
  labs(title = "Frequencies of Different Proteins in Recipes", x = "", y = "Number of Recipes") 

proteins <- proteins %>%
  mutate(protein = fct_reorder(protein, -n))

p2 <- ggplot(proteins, aes(x = protein, y = avg_rating, fill = protein)) + 
  geom_col() + 
  theme_classic() + 
  theme(legend.position = "none") +
  labs(title = "Average Ratings of Recipes with Different Proteins", x = "", y = "Average Rating", caption = "Cate Stacy, 9/24/25, Allrecipes.com")
