...
```{r}
#1
diamonds_400 <- diamonds %>%
  filter(price < 400 | price > 10000)

#2
diamonds_500 <- diamonds %>%
  filter(price > 500 & price < 600)

#3
diamonds %>%
  mutate(fpi = cut %in% c("Fair", "Premium", "Ideal")) %>%
  summarise(num_fp1 = sum(fpi), frac_fpi = mean(fpi))

#4
diamonds %>%
  filter(cut == "Fair") %>%
  summarize(high = any(price > 3000))
diamonds %>%
  filter(cut == "Ideal") %>%
  summarize(high = all(price > 2000))
...
ratings_ordered <- fct(ratings, levels = c("Low", "Medium", "High"))
