...
```{r ex_create_string}
# Your code
strThurs <- "It's Thursday"

# strThurs2 <- 'It's Thursday'
...
df_dates <- df_dates %>%
  mutate(
    ...
    full_date1 = str_c(month_num, "-", day, "-", year), full_date2 = str_glue("{month_num}-{day}-{year}")
  )
...
df <- df %>%
  mutate(mid_letter = str_sub(word, start = floor(str_length(word)/2) + 1, end = floor(str_length(word)/2) + 1))
```
