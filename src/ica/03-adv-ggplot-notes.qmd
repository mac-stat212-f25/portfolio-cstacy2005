...
NYT wanted readers to explore how the experienced temperatures were abnormal in the context of historical temperatures.
  
  - How did this goal inform what information is displayed in the visualization?
This goal is why they included the normal ranges and records.

**Aesthetic Mapping** 

- What specific variables (from the data codebook) underlie the visualization?
Month, day, low/high temp, precip, recordPrecip 

- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?
There are different colors and shading for the actual temps, records, and normal ranges. The actual temps are most apparent and the other data is in the background.
...
```{r}
months <- data.frame(month = c(1:12), day = c(31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365), culmP_month = c(0.94, 4.79, 5.7, 0.33, 0.47, 1.49, 0, 0, 0.01, 1.18, 1.55, 0.13))
...
```
I found that I was able to figure out the right command through a Google search. I then looked up the specifications in the console. However, I was unable to get the code to run. So, I asked in ChatGPT "how to add custom x axis labels ggplot r" and in the bottom of the output, it said the command won't work if the number of labels doesn't match the number of breaks. So, I asked what to do if that is the case, and it showed me how I could chose to label only specific values.
