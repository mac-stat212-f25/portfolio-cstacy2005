...
```{r}   
months <- vector("character", length = length(month.name))

# for loop
for(i in seq_along(month.name)) {
  months[i] <- str_c("Month ", i, ": ", month.name[i])
}
months
...
models <- vector(mode = "list", length = length(cuts))

for(i in seq_along(cuts)) {
  curr_cut <- cuts[i]
  diamonds_sub <- diamonds %>%
    filter(cut == curr_cut)
  models[[i]] <- lm(price ~ carat, data = diamonds_sub)
}
```
...
diamonds %>%
  mutate(across(.cols = c(x, y, z), .fns = mm))
```
...
```{r}
all_models <- vector(mode = "list", length = nrow(df_arg_combos))

for(i in seq_along(all_models)) {
  curr_formula <- df_arg_combos$mod_formula[[i]]
  curr_outliers <- df_arg_combos$remove_outliers[i]
  all_models[[i]] <- fit_model(diamonds, curr_formula, curr_outliers)
}
