...
```{r}
rescale_num <- function(x, remove_nas = TRUE) {
...
```{r}
digit_format <- function(x, remove_nas = TRUE) {
  x <- str_remove_all(x, "[^[:alnum:]]")
  chunk1 <- str_sub(x, 1, 3)
  chunk2 <- str_sub(x, 4, 6)
  chunk3 <- str_sub(x, 7, 10)
  return(paste0("(", chunk1, ") ", chunk2, "-", chunk3))
}

digit_format(6516966000)
digit_format(c("651-330-8661"))
digit_format("800 867 5309")
```
...
Order is wrong. For geom_sf, mapping should come before data.
...
domain_name <- function(email) {
  if(!str_detect(email, "@.+\\..+")){
    ...
    index = str_locate(email, "@")
    return(str_sub(email, index[1] + 1))
  }
}

domain_name("cstacy1@macalester.edu")
domain_name("oranges25")
```
...
```{r}
new_count <- function(df, var) {
  df %>%
    count(pick({{ var }})) %>%
    mutate(prop = n/sum(n))
}

new_count(diamonds, var = cut)
new_count(diamonds, var = c(cut, color))
```
...
```{r}
manual_scatter <- function(df, x, y) {
...
manual_scatter(diamonds, x = carat, y = price)
```
## Done!
