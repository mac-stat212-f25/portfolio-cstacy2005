...
ggplot(airquality_mod, aes(x = Solar.R_mod2, y = Ozone)) +
    ...
    labs(title = "Alternate Situation 2")
```
In both Alternate Situations, the Solar R for when ozone is NA is much higher than when ozone is not NA. In Alternate Situation 1 scatterplot, we see that the missing values are all at higher values of Solar R. Because we have observed data within the range of these missing values, we could fit a relationship. The scatterplot in Alternate Situation 2 shows that many of the missing ozone values are beyond the range of Solar R. A model may not be appropriate because we don't have a good sense of the realtionship.

Our actual situation does not show a relationship between missing ozone and Solar R as the bars are much more randomly distributed.
...
```{r}
ggplot(airquality, aes(x = Month, fill = is.na(Ozone))) + 
...
```
Missingness is more common in June. Ozone values are higher in July and August. Ozone values are lower when wind is higher and temp is lower.

Yes, it looks like a linear regression model would be helpful because the missing values occur where we have data for the other variables and the overall trend looks linear.
